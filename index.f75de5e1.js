...
const promise2 = new Promise((resolve, reject)=>{
    ...
    body.addEventListener("contextmenu", ()=>resolve("Second promise was resolved"));
});
...
const successful = (message)=>{
    body.insertAdjacentHTML("beforeend", `<div class="success" data-qa="notification">${message}</div>`);
};
const error = (message)=>{
    body.insertAdjacentHTML("beforeend", `<div class="warning" data-qa="notification">${message}</div>`);
};
